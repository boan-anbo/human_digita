...
from human_digita.independent_serializers.AnnotationSerializerWithoutPassages import AnnotationSerializerWithoutPassages
...
class PassageSerializer(serializers.HyperlinkedModelSerializer):
    ...
    annotations = AnnotationSerializerWithoutPassages(many=True, read_only=True)
    class Meta:
        model = Passage
        fields = [
            'id',
...
class PassageIndexSerializer(HaystackSerializer):
    class Meta:
        ...
        fields = ['text', 'id', 'language', 'title']  # text 由索引类进行返回, object 由序列化类进行返回,第一个参数必须是text
